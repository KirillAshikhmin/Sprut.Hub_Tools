window.STATE =
{
  ...
  "concerns": [
    ...
    "код 01 · ExhaustFanAutomation.js:895 — clearAutoOnLockTimer скрыто гасит семантический флаг autoOnLockSilent",
    "код 01 · ExhaustFanAutomation.js:148,377,711 — решение «нужен ли таймер выключения» живёт в трёх местах плюс четвёртая проверка внутри armOffTimer",
    "код 01 · ExhaustFanAutomation.js:440,450 — computeOccupancyActive заново кодирует активные значения вместо общей карты «тип → характеристика»",
    "код 01 · ExhaustFanAutomation.js:18,516 — PRESENCE_READY_TOLERANCE_MS=50 затыкает два источника истины о «присутствие набрано»",
    ...
    "код 01 · признак стартового вызова косвенный (!variables.externalSubscribed) — прямого признака хаб не даёт; при отключённом onStart первое событие будет принято за запуск",
    ...
    "код 01 · ExhaustFanAutomation.js:101 — признак старта обязан стоять до ensureExternalSubscription: временная связанность",
    ...
    "код 01 · гонка: событие On/Active, доставленное раньше стартового вызова, один раз сойдёт за запуск (переключение не теряется)",
