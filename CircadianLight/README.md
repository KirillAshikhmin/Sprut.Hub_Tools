...
#### Режимы рассвета и заката
Плавный сценарный переход, полезный как «будильник светом» или «уход ко сну». Вызываются из Блока кода:
```
global.enableSunriseModeFor(service)          // рассвет
global.enableSunsetModeFor(service)           // закат (по умолчанию гасит лампу в конце)
global.enableSunsetModeFor(service, true)     // закат с удержанием финального значения
global.disableModeFor(service)                // отменить режим досрочно (вернуться к циркаде)
```
где `service` — `Hub.getAccessory(aId).getService(sId)` (можно передать массив сервисов).

- **Рассвет** — за ~30 минут плавно поднимает яркость с 1% и тёплого света к текущим циркадным значениям; выключенную лампу включает сам. По завершении режим снимается, лампа продолжает работать по циркаде.
- **Закат** — за ~30 минут плавно уводит к 1% яркости и тёплому свету. По завершении **по умолчанию выключает лампу**. Если передать `keepOn = true`, лампа остаётся на финальном значении, а циркада останавливается — до ручного выключения, вызова `global.resetCircadianLight(service)` или ручного изменения характеристики.

Значения меняются шагами ~30 секунд (плавно). Длительность режимов задаётся в `GlobalVariables.CIRCADIAN_LIGHT_MODES_CONFIG` (глобальный сценарий «Параметры»).

> Состояние активного режима хранится в `GlobalVariables` и **сбрасывается при перезагрузке хаба** — начатый рассвет/закат при рестарте не возобновляется.

#### Динамические режимы
Для того, что бы менять значение температуры и яркости, устанавливаемые лампой, необходлимо в глобальном сценарии (в новом или в конце "Циркадное освещение. Глобальный. Параметры") функцию.
```
 ...
 * serviceUUID - идентификатор сервиса в виде строки "АЕСЕССУАР.СЕРВИС", напримре "222.13"
 ...
 И внутри этой функции уже возвращать нужное значение в формате массива из 2 объектов: `[temp, bright]`. Функция вызывается для рассчёта устанавливаемого значения каждый раз непосредственно перед установкой. В конце обязательно возвращать `return [temp, bright];` 
...
# История изменений:
## 7.0
- Режимы «Рассвет» и «Закат»: плавное изменение (шаги ~30 c) с корректным завершением — рассвет возвращается к циркаде, закат гасит лампу или (при `keepOn`) держит финальное значение до ручного действия.
- Управление (отключение/включение/сброс/смена режима) переведено с 1-секундного опроса глобальных переменных на коллбеки — мгновенно, без таймера на каждую лампу.
- Реестр коллбеков самоинициализируется с обеих сторон — управление работает сразу после перезагрузки хаба.
- Автоочистка обработчиков удалённых ламп, отписка и широковещательный вызов; функции управления возвращают признак доставки команды.
- Поведение при ручном изменении: добавлен вариант «Восстановить циркадное значение сразу».
- Для связанных ламп добавлен вариант «Восстановить циркадное значение».
- Определение способа включения и обработка ручных изменений переработаны — надёжнее и читаемее.
- «Отключение циркадного освещения»: выбор до 10 ламп, управление через коллбеки.
- Добавлены юнит-тесты логической и глобальной части.

## 6.0
- Доработано поведение при изменении характеристики лампы
- Реализована опция поведение при изменении характеристики у связанной лампы (Например люстры из умных ламп)
- Опция "Менять температуру после яркости"

## 5.1
- Возможность создавать динамические режимы (устанавливать температуру и яркость в зависимости от своих условий)
- Параметр "Случайное время изменения". С этим параметром значение будет устанавливаться в 5 минут и 0-59 секунд. Необходимо если сценарий активирован у многих ламп, что бы избежать ошибки 'Очередь на выполнение не может быть больше 5-и значений'
- Исправление ситуации, когда на связанной лампе изменяется значение с задержкой, и оно приходит на ту, на которой включена логика и тем самым останавливает изменение ярости или температуры.
- Исправление проблемы, когда логика пыталась изменить значение на лампе когда менять ничего нельзя.
- Значения характеристики теперь устанавливаются всегда, а не смотрит прошле изменение, что бы всегда было актуальное (особенно важно в ситуации, когда лампа была в цветном режиме, а надо перейти в режим температуры, но значение температуры стояло верное)
- Для глобального сценария написал Unit-тест  что бы не допускать ошибок :)

## 5.0
- Новый способ определения ручного измененения характеристик лампы для остановки сценария (через контект)
- Исправлена проблема, когда изменения яркости и насыщенности у ламп, у которых есть характеристика цветовой температуры, что приводило к изменению цвета или отключению лампы (теперь точно)
- Режим плавного включения исправлен, а так же параметр изменён на время включения в секундах
- В выбор режимов добавлены несколько кастомных, что бы можно было выбрать их после добавления. Имя подгрузится после перезагрузки хаба\обновления логического сценария.
- В сценарии "Отключение циркадного освещения" теперь можно выбрать и скрытые лампы


## 4.3
- Новый логический сценарий "Отключение циркадного освещения" для выключателей - включает и отключает режим циркадного освещения у указанных ламп (до 10 штук)
- В настройках сценария отображается описание
- Исправлена проблема, когда изменения яркости и насыщенности у ламп, у которых есть характеристика цветовой температуры, что приводило к изменению цвета или отключению лампы
- Отображение событий автоматического изменения характеристик в отладке

## 4.2
- Исправлена проблема, когда при включении лампы циркадный режим автоматически отключался изменением параметров
- Вывод контекста в лог при отладке при изменения характеристики, которая вызывает остановку работы режима

## 4.1
- Исправление автоматического включения лампы при вызове сброса или включения циркадного режима через вызов глобальной функции enableCircadianLightFor или resetCircadianLightFor
- Лог при сбросе циркадного режима
