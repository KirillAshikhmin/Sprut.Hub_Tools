...
function trigger(source, value, variables, options, context) {
    ...
    try {
        ...
        // Определяем, нужно ли искать переключатель внешнего датчика
        const needsSwitch = thermostatConfig ? thermostatConfig.hasSwitch : true;

        // Поиск сервисов в зависимости от типа термостата
        acc.getServices().forEach(function (service) {
            if (needsSwitch && service.getType() === HS.Switch && !externalSwitch) {
