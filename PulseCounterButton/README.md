...
текущими значениями. Если счётчик был недоступен на старте (offline) и база не
инициализировалась, его первое появление с ненулевым значением трактуется как
нажатие. Собственная запись в кнопку отсекается проверкой контекста
(`self-change`), поэтому цикла не возникает.

## Опции

- **ОПИСАНИЕ** (`desc`) — информационный блок в настройках логики (описание сценария), на работу не влияет.
