# В данном разделе собрана документация по логическим сценариям для Sprut.Hub, а также шаблон для них

Сценарии пишутся на JavaScript для движка Nashorn; поддерживается только стандарт ES5 (ECMAScript 5) с некоторыми возможностями из ES6.
...
- Map и Set коллекции
- Цикл for...of
Все остальные функции (деструктуризация, классы, модули, промисы и т.д.) не поддерживаются.
...
YAGNI (You Aren't Gonna Need It) - не добавляй функциональность заранее
KISS (Keep It Simple, Stupid) - простота решения в приоритете


## Дополнительная информация
Файл [sh_types.json](./sh_types.json) — содержит список всех доступных сервисов в хабе со всеми свойствами, а также характеристики, которые могут содержаться в этом сервисе и их формат и допустимые значения.

Файл [spruthub.js](./spruthub.js) — JS API Sprut.Hub со всеми доступными в нём функциями и документацией на русском языке.
...
Структура устройств: каждое устройство — это аксессуар (Accessory). В каждом аксессуаре может быть один или более сервисов (Service). В сервисе — одна или более характеристик (Characteristic). Для каждого сервиса список доступных характеристик ограничен. Узнать какие характеристики могут быть в сервисе можно изучив файл sh_types.json или посмотрев в интерфейсе хаба в разделе Настройки - Типы сервисов. Аксессуар может содержать неограниченное количество сервисов с одинаковыми или разными типами. Характеристики же не более 1 штуки каждого поддерживаемого типа в сервисе.
...
5. **Enum** — выбор из списка (тип может быть любым из перечисленных выше; важно, чтобы значение в поле value опции и допустимые значения были того же типа). Отличие от других типов: нужно указать values со списком допустимых значений и `formType: "list",`
...
#### trigger
Основная функция сценария и точка входа. Вызывается системой после выполнения compute и фактической установки значения в хабе. При включённом `onStart` в блоке info вызывается при запуске хаба и при сохранении сценария с текущим значением характеристики. Работает асинхронно.
Должна располагаться после блока info перед остальными функциями. Допускается размещение констант как над блоком info, так и над функцией trigger.
```javascript
function trigger(source, value, variables, options, context) {
  // Код сценария
}
