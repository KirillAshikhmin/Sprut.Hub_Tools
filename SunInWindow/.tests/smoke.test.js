// Смоук-тест сценария "☀️ Солнце в окне" (SunInWindow).
//
...
// ---------------------------------------------------------------------------
// §5, поправка D02: любая пустая форма windowAzimuth значит "не задан" — берётся
// румб, ошибки нет, таймер ставится. Пустая строка и строка из пробелов ведут себя
// ровно как -1.
// ---------------------------------------------------------------------------

describe('§5 (D02) Пустой точный азимут', () => {
  const blanks = ['', '   '];
  for (const blank of blanks) {
    it('windowAzimuth = "' + blank + '" → направление берётся из румба, как при -1', ({ hub, scenario, time, logs }) => {
      time.set('2026-06-21T09:31:00Z');   // солнце на юге, высота 57,7°
      const sw = makeSwitch(hub, 10, false);
      const on = sw.char(HS.Switch, HC.On);

      scenario.run({
        source: on, value: false, variables: freshVars(),
        options: baseOptions({ windowAzimuth: blank, windowDirection: 180, maxAzimuthDeviation: 5 }),
        context: '',
      });

      expect(on.getValue()).toBe(true);
      expect(logs.byLevel('error')).toHaveLength(0);
