...
## Амендменты (по ходу реализации)

Отклонения от исходных код-блоков плана, внесённые при выполнении (код в репозитории — источник истины; блоки выше могут не отражать эти правки дословно):

- **Task 1** (`fa7a758`): `isInfoAssignment` для формы `var info = …` теперь требует, чтобы инициализатор был `ObjectExpression` (иначе `var info = helper()` + `trigger` ложно классифицировался как LOGIC). `AnyNode.declarations` расширен полем `init`. +2 регрессионных теста.
- **Task 5** (`c8a9bac`): тест zip переписан на портируемый — старый macOS `unzip` 6.00 отображает UTF-8-имена как mojibake в `-l`, поэтому реальную распаковку проверяем на ASCII-именах, а UTF-8-флаг и байты кириллического имени — побайтово. `version needed` в zip.ts = 20.
- **Task 6** (`eb0f6f9`): убран неиспользуемый импорт `resolve` из `node:path`.
- **Task 7** (`d5ac028`): в фикстуре `fixtures/ok/source/Ok.js` `info.name` изменён с «Ок сценарий» на «Ok» — при отсутствии совпадающего JSON `deriveManifest` берёт имя выходного файла из `info.name`, поэтому тест, ожидающий `Ok.json`, требует `info.name === "Ok"` (исходный план был внутренне несогласован).
- **Task 8** (`39be891`, вскрыто smoke-прогонами):
  - `parseInfo` теперь резолвит одноуровневые `IDENT.prop`, где `IDENT` — верхнеуровневый объект-литерал (DRY-паттерн `description: scenarioDescription.ru`). Иначе VirtualThermostat давал ложную ошибку `info-nonliteral`. +2 теста.
  - `deriveManifest` больше НЕ выбирает primary вслепую (`?? logicCandidates[0]`): при нескольких LOGIC без совпадения имени с папкой primary не ставится, и `resolveVersion` честно требует уточнить его при разных версиях. Иначе CircadianLight (3 LOGIC: 1.0/7.0/1.1) ложно проходил changelog по v1.0, маскируя отсутствие записи 7.0. +1 тест, фикстуры `fixtures/multi/`.
