...
if prev === undefined → prev = 0                            // счётчик впервые виден (нет базы) — считаем от 0, ниже даст событие
...
| Счётчик впервые виден (`prev` пуст, недоступен на старте) | считаем от 0 → первое ненулевое значение даёт событие          |
